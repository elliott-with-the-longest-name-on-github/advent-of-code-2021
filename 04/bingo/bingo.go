...
func SubsystemFromText(plays []int, boards [][]string) (*BingoSubsystem, error) {
	...
	for _, board := range boards {
		bingoBoard, err := BoardFromText(board)
		if err != nil {
			return nil, fmt.Errorf("failed to parse board with error %s", err)
		}
		bingoBoards = append(bingoBoards, bingoBoard)
	}
	subsystem := &BingoSubsystem{
		Plays:  plays,
		Boards: bingoBoards,
	}
...
func (b *BingoSubsystem) PlayUntilAllBoardsWin() {
	for _, num := range b.Plays {
		b.PlayNumber(num)
		if b.LastWinningBoard != nil {
			break
		}
	}
}

func (b *BingoSubsystem) PlayNumber(number int) {
	if b.LastWinningBoard != nil {
		return
	}

	numBoards := len(b.Boards)
	for _, board := range b.Boards {
		if !board.Won {
			won := board.PlayNumber(number)
			if won {
				if b.WinningBoard == nil {
					b.WinningBoard = board
				}
				b.FinishedBoards++
			}
			if b.FinishedBoards == numBoards {
				b.LastWinningBoard = board
			}
		}
	}
	return
...
func BoardFromText(rows []string) (*BingoBoard, error) {
	board := &BingoBoard{Board: [5][5]BingoNumber{}}
	for i, row := range rows {
		numbers := strings.Fields(row)
		if len(numbers) != 5 {
			return nil, fmt.Errorf("rows must be 5 numbers wide")
		}
		for j, n := range numbers {
			number, err := strconv.Atoi(n)
			if err != nil {
				return nil, fmt.Errorf("failed to parse a bingo board number with error %s", err)
			}
			bNumber := BingoNumber{Value: number, Marked: false}
			board.Board[i][j] = bNumber
		}
	}
	return board, nil
}

func (b *BingoBoard) PlayNumber(number int) (won bool) {
	if b.Won {
		return true
	}
	for i, row := range b.Board {
		for j, v := range row {
			if v.Value == number {
				...
				won = b.DoesColumnWin(j) || b.DoesRowWin(i)
				b.Won = won || b.Won
			}
		}
	}
	b.LastNumber = BingoNumber{Value: number, Marked: true}
	return
}

func (b *BingoBoard) DoesColumnWin(col int) bool {
	won := true
	for _, row := range b.Board {
		won = row[col].Marked && won
	}
	return won
}

func (b *BingoBoard) DoesRowWin(row int) bool {
	won := true
	boardRow := b.Board[row]
	for _, bNum := range boardRow {
		won = bNum.Marked && won
	}
	return won
}
...
var ChallengeBoards [][]string = [][]string{{
	"92  3 88 13 50",
	"90 70 24 28 52",
	"15 98 10 26  5",
	"84 34 37 73 87",
